...
def test_format_qs():
    qs = format_qs(
        {"foo": "bar", "_tags": "runtime:dev", "status": "completed,running"}
    )
    assert qs == "?foo=bar&_tags=runtime:dev&status=completed,running"


def test_format_baseurl():
    request = make_mocked_request("GET", "/foo/bar?foo=bar", headers={"Host": "test"})
...
def test_db_conf_dsn():
    with set_env():
        assert DBConfiguration(dsn="user=foo").get_dsn() == "user=foo"


def test_db_conf_arguments():
    with set_env():
        db_conf = DBConfiguration(
            host="foo", port=1234, user="user", password="password", database_name="bar"
        )
        assert (
            db_conf.get_dsn()
            == "dbname=bar user=user host=foo port=1234 password=password"
        )
...
def test_db_conf_env_dsn():
    with set_env({"MF_METADATA_DB_DSN": "foo"}):
        ...
        assert (
            DBConfiguration().get_dsn()
            == "dbname=postgres user=postgres host=localhost port=5432 password=postgres"
        )

    with set_env(
        {
            "MF_METADATA_DB_DSN": "dbname=testgres user=test_user host=test_host port=1234 password=test_pwd"
        }
    ):
        # valid DSN in env should set correctly.
        assert (
            DBConfiguration().get_dsn()
            == "dbname=testgres user=test_user host=test_host port=1234 password=test_pwd"
        )

    with set_env(
        {
            "MF_METADATA_DB_DSN": "dbname=testgres user=test_user host=test_host port=1234 password=test_pwd sslmode=verify-full sslrootcert=/test"
        }
    ):
        # valid DSN in env should set correctly.
        assert (
            DBConfiguration().get_dsn()
            == "dbname=testgres user=test_user host=test_host port=1234 password=test_pwd sslmode=verify-full sslrootcert=/test"
        )
