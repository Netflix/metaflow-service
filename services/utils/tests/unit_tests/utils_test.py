...
def test_db_conf():
    with set_env():
        ...
        assert db_conf.get_dsn() == 'dbname=postgres user=postgres host=localhost port=5432 password=postgres'
...
def test_db_conf_arguments():
    with set_env():
        ...
        assert db_conf.get_dsn() == 'dbname=bar user=user host=foo port=1234 password=password'
...
def test_db_conf_env_dsn():
    with set_env({'MF_METADATA_DB_DSN': 'foo'}):
        ...
        assert DBConfiguration().get_dsn() == 'dbname=postgres user=postgres host=localhost port=5432 password=postgres'
