...
async def test_list_metadata(cli, db):
    ...
    await _test_list_resources(
        cli, db, "/flows/{flow_id}/runs/{run_number}/metadata".format(**_task), 200, []
    )
    await _test_list_resources(
        cli,
        db,
    ...
    _metadata = (
        await add_metadata(
            ...
            metadata={"field_name": "field_name", "value": "value", "type": "type"},
        )
    ).body
...
async def test_list_metadata_field_names(cli, db):
    ...
    await _test_list_resources(
        ...
        200,
        [_metadata_second, _metadata_first],
        ignore_data_order=True,
    )
...
async def test_list_metadata_attempt_id_filter(cli, db):
    ...
    _metadata_second = (
        await add_metadata(
            ...
            task_id=_task.get("task_id"),
            task_name=_task.get("task_name"),
            metadata={"field_name": "attempt", "value": "1", "type": "attempt"},
            tags=["attempt_id:1"],
        )
    ).body
    ...
    await _test_list_resources(
        cli,
        db,
        "/flows/{flow_id}/runs/{run_number}/metadata?_order=ts_epoch".format(**_task),
        200,
        [_metadata_second, _metadata_first, _metadata_global],
    )
    await _test_list_resources(
        cli,
        db,
        "/flows/{flow_id}/runs/{run_number}/steps/{step_name}/tasks/{task_id}/metadata?_order=ts_epoch".format(
            **_task
        ),
        200,
        [_metadata_second, _metadata_first],
    ...
    await _test_list_resources(
        cli,
        db,
        "/flows/{flow_id}/runs/{run_number}/metadata?_order=ts_epoch&attempt_id=0".format(
            **_task
        ),
        200,
        [_metadata_first],
    ...
    await _test_list_resources(
        cli,
        db,
        "/flows/{flow_id}/runs/{run_number}/steps/{step_name}/tasks/{task_id}/metadata?_order=ts_epoch&attempt_id=0".format(
            **_task
        ),
        200,
        [_metadata_first],
    ...
    await _test_list_resources(
        cli,
        db,
        "/flows/{flow_id}/runs/{run_number}/metadata?_order=ts_epoch&attempt_id=0,1".format(
            **_task
        ),
        200,
        [_metadata_second, _metadata_first],
    ...
    await _test_list_resources(
        cli,
        db,
        "/flows/{flow_id}/runs/{run_number}/steps/{step_name}/tasks/{task_id}/metadata?_order=ts_epoch&attempt_id=0,1".format(
            **_task
        ),
        200,
        [_metadata_second, _metadata_first],
    ...
    await _test_list_resources(
        cli,
        db,
        "/flows/{flow_id}/runs/{run_number}/metadata?_order=ts_epoch&attempt_id:is=null".format(
            **_task
        ),
        200,
        [_metadata_global],
    ...
    await _test_list_resources(
        cli,
        db,
        "/flows/{flow_id}/runs/{run_number}/steps/{step_name}/tasks/{task_id}/metadata?_order=ts_epoch&attempt_id:is=null".format(
            **_task
        ),
        200,
        [],
