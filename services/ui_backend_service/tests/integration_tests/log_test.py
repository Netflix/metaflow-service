...
async def test_log_default_response(cli, db):
    ...
    with mock.patch(
        "services.ui_backend_service.api.log.read_and_output", new=read_and_output
    ):
        _, data = await _test_list_resources(
            cli,
            db,
            "/flows/{flow_id}/runs/{run_number}/steps/{step_name}/tasks/{task_id}/logs/out".format(
                **_task
            ),
            200,
            None,
        ...
        _, data = await _test_list_resources(
            cli,
            db,
            "/flows/{flow_id}/runs/{run_number}/steps/{step_name}/tasks/{task_id}/logs/out?attempt_id=0".format(
                **_task
            ),
            200,
            None,
        ...
        await _test_list_resources(
            cli,
            db,
            "/flows/{flow_id}/runs/{run_number}/steps/{step_name}/tasks/{task_id}/logs/out?attempt_id=1".format(
                **_task
            ),
            404,
            None,
...
async def test_log_paginated_response(cli, db):
    ...
    with mock.patch(
        ...
        _, data = await _test_list_resources(
            cli,
            db,
            "/flows/{flow_id}/runs/{run_number}/steps/{step_name}/tasks/{task_id}/logs/out?_limit=3".format(
                **_task
            ),
            200,
            None,
    ...
    with mock.patch(
        "services.ui_backend_service.api.log.read_and_output", new=read_and_output_2
    ):
        # ordering by row should be possible in reverse. should obey limit.
        _, data = await _test_list_resources(
            cli,
            db,
            "/flows/{flow_id}/runs/{run_number}/steps/{step_name}/tasks/{task_id}/logs/out?_order=-row&_limit=3&_page=4".format(
                **_task
            ),
            200,
            None,
...
async def test_log_download_response(cli, db):
    ...
    with mock.patch(
        ...
        resp = await cli.get(
            "/flows/{flow_id}/runs/{run_number}/steps/{step_name}/tasks/{task_id}/logs/out/download".format(
                **_task
            )
        )
