...
async def test_task_not_found_response(cli, db):
    _, data = await _test_list_resources(
        cli, db, "/flows/DummyFlow/runs/123/steps/start/tasks/456/cards", 404, None
    )
    assert data == []

    resp = await cli.get(
        "/flows/DummyFlow/runs/123/steps/start/tasks/456/cards/abcd123"
    )
...
async def test_card_not_returned(cli, db):
    ...
    with mock.patch(
        ...
        new=get_cards_for_task,
    ):
        resp = await cli.get(
            "/flows/{flow_id}/runs/{run_number}/steps/{step_name}/tasks/{task_id}/cards/abcd1234".format(
                **_task
            )
        )
        body = await resp.text()
...
async def test_cards_list_paginated_response(cli, db):
    ...
    with mock.patch(
        ...
        _, data = await _test_list_resources(
            cli,
            db,
            "/flows/{flow_id}/runs/{run_number}/steps/{step_name}/tasks/{task_id}/cards?_limit=1".format(
                **_task
            ),
            200,
            None,
        ...
        _, data = await _test_list_resources(
            cli,
            db,
            "/flows/{flow_id}/runs/{run_number}/steps/{step_name}/tasks/{task_id}/cards?_limit=1&_page=2".format(
                **_task
            ),
            200,
            None,
...
async def test_card_content_for_task(cli, db):
    ...
    with mock.patch(
        ...
        resp = await cli.get(
            "/flows/{flow_id}/runs/{run_number}/steps/{step_name}/tasks/{task_id}/cards/hij123".format(
                **_task
            )
        )
        assert resp.status == 404
        ...
        resp = await cli.get(
            "/flows/{flow_id}/runs/{run_number}/steps/{step_name}/tasks/{task_id}/cards/abc".format(
                **_task
            )
        )
        body = await resp.text()
        ...
        resp = await cli.get(
            "/flows/{flow_id}/runs/{run_number}/steps/{step_name}/tasks/{task_id}/cards/def".format(
                **_task
            )
        )
        body = await resp.text()
