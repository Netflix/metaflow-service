...
async def test_list_steps(cli, db):
    ...
    await _test_list_resources(
        cli, db, "/flows/{flow_id}/runs/{run_number}/steps".format(**_run), 200, []
    )
    ...
    _, data = await _test_list_resources(
        cli, db, "/flows/{flow_id}/runs/{run_number}/steps".format(**_step), 200, None
    )
...
async def test_single_step(cli, db):
    await _test_single_resource(
        cli, db, "/flows/HelloFlow/runs/404/steps/none", 404, {}
    )
