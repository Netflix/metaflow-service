...
async def test_list_tasks(cli, db):
    ...
    await _test_list_resources(
        cli, db, "/flows/{flow_id}/runs/{run_number}/tasks".format(**_step), 200, []
    )
    await _test_list_resources(
        cli,
        db,
    ...
    await _test_list_resources(
        cli,
        db,
        "/flows/{flow_id}/runs/{run_number}/tasks".format(**_task),
        200,
        [_task],
    )
    await _test_list_resources(
        cli,
        db,
        "/flows/{flow_id}/runs/{run_number}/steps/{step_name}/tasks".format(**_task),
        200,
        [_task],
    )
...
async def test_list_tasks_non_numerical(cli, db):
    ...
    _, data = await _test_list_resources(
        cli, db, "/flows/{flow_id}/runs/{run_number}/tasks".format(**_task), 200, None
    )
    _, data = await _test_list_resources(
        cli,
        db,
...
async def test_single_task(cli, db):
    await _test_single_resource(
        cli, db, "/flows/HelloFlow/runs/404/steps/none/tasks/5", 404, {}
    )
...
async def test_single_task_non_numerical(cli, db):
    ...
    _, data = await _test_single_resource(
        cli,
        db,
        "/flows/{flow_id}/runs/{run_number}/steps/{step_name}/tasks/bar".format(
            **_task
        ),
        200,
        None,
...
async def test_list_old_metadata_task_attempts(cli, db):
    ...
    await _test_list_resources(
        cli,
        db,
        "/flows/{flow_id}/runs/{run_number}/steps/{step_name}/tasks/{task_id}/attempts".format(
            **_task
        ),
        200,
        [_task],
    ...
    _task_first_attempt["duration"] = (
        _artifact_first["ts_epoch"] - _task_first_attempt["ts_epoch"]
    )

    _task_second_attempt["attempt_id"] = 1
    _task_second_attempt["finished_at"] = _artifact_second["ts_epoch"]
    _task_second_attempt["duration"] = (
        _artifact_second["ts_epoch"] - _task_second_attempt["ts_epoch"]
    )

    await _test_list_resources(
        cli,
        db,
        "/flows/{flow_id}/runs/{run_number}/steps/{step_name}/tasks?task_id={task_id}".format(
            **_task
        ),
        200,
        [_task_second_attempt, _task_first_attempt],
    )
...
async def test_old_metadata_task_with_multiple_attempts(cli, db):
    ...
    _task["duration"] = _artifact_second["ts_epoch"] - _task["ts_epoch"]
...
async def test_task_with_attempt_metadata(cli, db):
    ...
    _task["task_ok"] = (
        None  # intended behavior, status refinement location field should remain empty when metadata exists.
    )
...
async def test_task_failed_status_with_heartbeat(cli, db):
    ...
    _task["finished_at"] = (
        1000  # should be last heartbeat in this case, due to every other timestamp missing.
    )
...
async def test_list_task_attempts(cli, db):
    ...
    _task_first_attempt["duration"] = (
        _task_first_attempt["finished_at"] - _task_first_attempt["started_at"]
    )
    ...
    _task_second_attempt["finished_at"] = _artifact_second["ts_epoch"]
    _task_second_attempt["duration"] = (
        _task_second_attempt["finished_at"] - _task_second_attempt["started_at"]
    )
...
async def test_list_task_multiple_attempts_failure(cli, db):
    ...
    _attempt_ok_second = await create_task_attempt_ok_metadata(
        db, _task, 1, True
    )  # status = 'completed'
    ...
    _task_first_attempt["duration"] = (
        _attempt_ok_first["ts_epoch"] - _task_first_attempt["started_at"]
    )
    ...
    _task_second_attempt["duration"] = (
        _attempt_ok_second["ts_epoch"] - _task_second_attempt["started_at"]
    )
...
async def test_task_attempts_with_attempt_metadata(cli, db):
    ...
    _task_first_attempt["attempt_id"] = 0
    _task_first_attempt["task_ok"] = (
        "location"  # should have location for status artifact
    )
    _task_first_attempt["status"] = (
        "unknown"  # 'unknown' because we cannot determine correct status from DB as attempt_ok is missing
    )
    _task_first_attempt["started_at"] = _attempt_first["ts_epoch"]
    _task_first_attempt["finished_at"] = _attempt_done_first["ts_epoch"]
    _task_first_attempt["duration"] = (
    ...
    _task_second_attempt["started_at"] = _attempt_second["ts_epoch"]
    _task_second_attempt["duration"] = (
        int(round(time.time() * 1000)) - _task_second_attempt["started_at"]
    )
    ...
    _task_first_attempt["task_ok"] = (
        None  # should have no task_ok location, as status can be determined from db.
    )
    _task_first_attempt["status"] = (
        "failed"  # 'failed' because now we have attempt_ok false in db.
    )
...
async def test_task_attempt_status_running(cli, db):
    _task = await create_task(
        db, last_heartbeat_ts=get_heartbeat_ts()
...
async def test_task_attempt_status_failed_with_existing_subsequent_attempt(cli, db):
    ...
    _first_attempt["duration"] = (
        _first_attempt["last_heartbeat_ts"] * 1000 - _first_attempt["started_at"]
    )
    await _test_list_resources(
        cli,
        db,
    ...
    _second_attempt["duration"] = (
        _second_attempt["last_heartbeat_ts"] * 1000 - _second_attempt["started_at"]
    )
    ...
    _first_attempt["duration"] = (
        _first_attempt["finished_at"] - _first_attempt["started_at"]
    )
    ...
    await _test_list_resources(
        ...
        200,
        [_second_attempt, _first_attempt],
    )
...
async def create_task(
    db, status="running", task_id=None, task_name=None, last_heartbeat_ts=None
):
...
async def create_task_attempt_metadata(db, task, attempt=0):
    ...
    return await create_metadata_for_task(
        db,
        task,
        metadata={"type": "attempt", "field_name": "attempt", "value": str(attempt)},
    )
...
async def create_task_attempt_ok_metadata(
    db, task, attempt_id: int, attempt_ok: bool = False
):
