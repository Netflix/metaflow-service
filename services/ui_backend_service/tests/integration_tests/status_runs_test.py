...
async def test_run_status_running_with_heartbeat(cli, db):
    ...
    _run = (
        await add_run(db, flow_id=_flow.get("flow_id"), last_heartbeat_ts=_heartbeat)
    ).body
...
async def test_run_status_failed_with_heartbeat_expired_and_failed_task(cli, db):
    ...
    _heartbeat = (
        get_heartbeat_ts() - 65
    )  # old heartbeat, enough to count as expired, but not older than cutoff
...
async def test_run_status_failed_with_heartbeat_expired_and_no_failed_tasks(cli, db):
    ...
    _heartbeat = (
        get_heartbeat_ts() - 60 * 60 * 24 * 7
    )  # heartbeat older than threshold to count run as stuck, even with no failed tasks
...
async def test_run_status_failed_with_retrying_task(cli, db):
    ...
    _run = (
        await add_run(
            db, flow_id=_flow.get("flow_id"), last_heartbeat_ts=_expired_heartbeat
        )
    ).body
    # even when a run has a heartbeat, it still requires a task that has failed via attempt_ok=false OR by an expired heartbeat.
    _step = (
    ...
    await db.task_table_postgres.update_row(
        ...
        update_dict={"last_heartbeat_ts": _expired_heartbeat},
    )
    ...
    await db.task_table_postgres.update_row(
        ...
        update_dict={"last_heartbeat_ts": get_heartbeat_ts()},
    )
