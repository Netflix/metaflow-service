...
async def test_list_runs_group_by_flow_id(cli, db):
    ...
    await _test_list_resources(
        cli,
        db,
        "/runs?_group=flow_id&_group_limit=1",
        200,
    ...
    await _test_list_resources(
        cli,
        db,
        "/runs?_group=flow_id&_group_limit=2&_limit=1&_order=%2Brun_number",
        200,
        first_runs[:2],
        approx_keys=["duration"],
    )
    ...
    await _test_list_resources(
        cli,
        db,
        "/runs?_group=flow_id&_order=run_number",
        200,
        [*first_runs[::-1][:10], *second_runs[::-1][:10]],
        approx_keys=["duration"],
    )
...
async def test_list_runs_group_by_user(cli, db):
    ...
    await _test_list_resources(
        cli,
        db,
        "/runs?_group=user&_order=%2Brun",
        200,
        [*second_runs[:10], *first_runs[:10]],
        approx_keys=["duration"],
    )
    ...
    await _test_list_resources(
        cli,
        db,
        "/runs?_group=user&&_order=%2Brun&_group_limit=1",
        200,
        [second_runs[0], first_runs[0]],
        approx_keys=["duration"],
    )
    ...
    await _test_list_resources(
        cli,
        db,
        "/runs?_group=user&&_order=%2Brun&_group_limit=2&_limit=1",
        200,
        second_runs[:2],
        approx_keys=["duration"],
    )
    ...
    for _ in range(n):
        _run = (
            await add_run(
                ...
                flow_id=flow_id,
                user_name=user,
                system_tags=["runtime:dev", "user:{}".format(user)],
            )
        ).body
        _run["run"] = _run["run_number"]
        _run["status"] = "running"
        _run["duration"] = max(
            int(round(time.time() * 1000)) - _run["ts_epoch"], 1
        )  # approx assert breaks in the odd case when duration==0
        _run["user"] = user
