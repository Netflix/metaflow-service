...
async def test_list_artifact(cli, db):
    ...
    await _test_list_resources(
        cli, db, "/flows/{flow_id}/runs/{run_number}/artifacts".format(**_task), 200, []
    )
    await _test_list_resources(
        cli,
        db,
...
async def test_list_artifact_attempt_ids(cli, db):
    ...
    for _ in range(1):
        _task = (
            await add_task(
                db,
                flow_id=steps[0].get("flow_id"),
                step_name=steps[0].get("step_name"),
                run_number=steps[0].get("run_number"),
    ...
    for _ in range(2):
        _task = (
            await add_task(
                db,
                flow_id=steps[1].get("flow_id"),
                step_name=steps[1].get("step_name"),
                run_number=steps[1].get("run_number"),
                run_id=steps[1].get("run_id"),
            )
        ).body
    ...
    update_objects_with_run_tags(
        "artifact", artifacts_visible_for_first_step, run_by_step_name["first"]
    )
    update_objects_with_run_tags(
        "artifact", artifacts_visible_for_second_step, run_by_step_name["second"]
    )
    ...
    await _test_list_resources(
        cli,
        db,
        "/flows/{flow_id}/runs/{run_number}/artifacts".format(**steps[1]),
        200,
        artifacts_visible_for_second_step,
    )
