import pytest
from .utils import init_app, set_env, cli
...
async def test_features_false(cli):
    with set_env({"FEATURE_ONE": "false"}):
        await expect_features(cli, {"FEATURE_ONE": False})
...
async def test_features_only(cli):
    with set_env({"FEATURE_FOO": "true", "ANOTHER_ENV_VAR": "bar"}):
        await expect_features(cli, {"FEATURE_FOO": True})
