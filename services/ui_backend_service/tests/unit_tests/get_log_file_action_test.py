...
async def test_paginated_result_oob_page(test_log):
    body = paginated_result(
        content_iterator=_list_iter(test_log),
        page=2,
        limit=2000,
        reverse_order=False,
        output_raw=False,
...
async def test_paginated_result_with_limit(test_log):
    ...
    assert [obj["line"] for obj in body["content"]] == list(
        "log line {}".format(i) for i in range(6, 11)
    )
...
async def test_paginated_result_ordering(test_log):
    ...
    assert [obj["line"] for obj in body["content"]] == [
        line for _, line in test_log[::-1]
    ]
...
async def test_log_cache_id_uniqueness():
    ...
    assert log_cache_id(first_task, "stdout") != log_cache_id(
        first_task_second_attempt, "stdout"
    )
...
async def test_lookup_id_uniqueness():
    ...
    assert lookup_id(first_task, "stdout", 0, 1, False, False) == lookup_id(
        first_task, "stdout", 0, 1, False, False
    )

    assert lookup_id(first_task, "stdout", 0, 1, False, False) != lookup_id(
        first_task_second_attempt, "stdout", 0, 1, False, False
    )

    assert lookup_id(first_task, "stdout", 0, 1, False, False) != lookup_id(
        second_task, "stdout", 0, 1, False, False
    )

    assert lookup_id(first_task, "stdout", 0, 1, False, False) != lookup_id(
        first_task, "stdout", 1, 1, False, False
    )

    assert lookup_id(first_task, "stdout", 1, 1, False, False) != lookup_id(
        first_task, "stdout", 1, 0, False, False
    )

    assert lookup_id(first_task, "stdout", 1, 1, False, False) != lookup_id(
        first_task, "stdout", 1, 1, True, False
    )

    assert lookup_id(first_task, "stdout", 1, 1, False, False) != lookup_id(
        first_task, "stdout", 1, 1, False, True
    )
...
datetime_expectations = [
    ...
    ("123", None),
    (
        datetime.datetime(2021, 10, 27, 0, 0, tzinfo=datetime.timezone.utc),
        1635292800000,
    ),
]
...
def test_tail_log_provider(m_get_log_size, m_get_log_content):
    ...
    for case in [
        ...
        provider = TailLogProvider(
            case["max_tail_chars"], max_log_size_in_kb=case["max_total_size"]
        )
        ...
        if case["expect_to_truncate"]:
            ...
            assert (
                tail_log_content[1:]
                == mock_log_content[
                    len(mock_log_content) - case["expected_tail_lines"] :
                ]
            )
        else:
...
def test_stream_sorted_logs_consecutively():
    ...
    raw_stream_a = [
        (start_a_ts + datetime.timedelta(seconds=i), f"A line {i}")
        for i in range(0, 10)
    ]

    start_b_ts = datetime.datetime(2021, 10, 27, 0, 10, tzinfo=datetime.timezone.utc)
    raw_stream_b = [
        (start_b_ts + datetime.timedelta(seconds=i), f"B line {i}")
        for i in range(0, 10)
    ]
    with TemporaryDirectory() as d:
        ...
        assert [line for _ts, line in results] == [
            line for _ts, line in (raw_stream_a + raw_stream_b)
        ]


def test_stream_sorted_logs_sandwiched():
    start_b_start_ts = datetime.datetime(
        2021, 10, 27, 0, 0, tzinfo=datetime.timezone.utc
    )
    raw_stream_b_start = [
        (start_b_start_ts + datetime.timedelta(seconds=i), f"B line {i}")
        for i in range(0, 10)
    ]
    ...
    start_b_end_ts = datetime.datetime(
        2021, 10, 27, 0, 0, 20, tzinfo=datetime.timezone.utc
    )
    raw_stream_b_end = [
        (start_b_end_ts + datetime.timedelta(seconds=i), f"B line {i}")
    ...
    with TemporaryDirectory() as d:
        ...
        assert [line for _ts, line in results] == [
            line for _ts, line in (raw_stream_b_start + raw_stream_a + raw_stream_b_end)
        ]
