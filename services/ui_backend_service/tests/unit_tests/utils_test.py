...
def test_format_response():
    request = make_mocked_request("GET", "/runs?_limit=10", headers={"Host": "test"})
    ...
    expected_response = {
        "data": {"foo": "bar"},
        "status": 200,
        "links": {"self": "http://test/runs?_limit=10"},
        "query": {"_limit": "10"},
    }
...
def test_format_response_list():
    ...
    expected_response = {
        ...
        "pages": {"self": 1, "first": 1, "prev": 1, "next": 2, "last": None},
        "query": {"_limit": "1", "_page": "1"},
    }
...
def test_format_response_list_next_page_null():
    ...
    expected_response = {
        ...
        "pages": {"self": 2, "first": 1, "prev": 1, "next": None, "last": None},
        "query": {"_limit": "10", "_page": "2"},
    }
...
def test_pagination_query_custom():
    request = make_mocked_request("GET", "/runs?_limit=5&_page=3&_order=foo&_group=bar")

    page, limit, offset, order, groups, group_limit = pagination_query(
        request=request, allowed_order=["foo"], allowed_group=["bar"]
    )
...
def test_pagination_query_custom_order_asc():
    ...
    _, _, _, order, _, _ = pagination_query(request=request, allowed_order=["foo"])
...
def test_builtin_conditions_query_tags_all():
    request = make_mocked_request("GET", "/runs?_tags=foo,bar")
...
def test_builtin_conditions_query_tags_all_explicit():
    request = make_mocked_request("GET", "/runs?_tags:all=foo,bar")
...
def test_builtin_conditions_query_tags_any():
    request = make_mocked_request("GET", "/runs?_tags:any=foo,bar")
...
def test_builtin_conditions_query_tags_likeall():
    request = make_mocked_request("GET", "/runs?_tags:likeall=foo,bar")
...
def test_builtin_conditions_query_tags_likeany():
    request = make_mocked_request("GET", "/runs?_tags:likeany=foo,bar")
...
def test_custom_conditions_query():
    ...
    for op, query in operators.items():
        ...
        request = make_mocked_request(
            "GET",
            "/runs?{0}=HelloFlow,AnotherFlow&_tags=foo&user_name=dipper&{0}=ThirdFlow".format(
                op
            ),
        )

        conditions, values = custom_conditions_query(request, allowed_keys=["flow_id"])
...
def test_custom_conditions_query_allow_any_key():
    request = make_mocked_request("GET", "/runs?flow_id=HelloFlow&status=completed")

    conditions, values = custom_conditions_query(request, allowed_keys=None)
...
def test_resource_conditions():
    path, query, _ = resource_conditions("/runs?flow_id=HelloFlow&status=running")
...
def test_filter_from_conditions_query():
    ...
    request = make_mocked_request(
        "GET", "/?run=test&ts_epoch:gt=1", headers={"Host": "test"}
    )
    ...
    request = make_mocked_request(
        "GET", "/?run=test,test_1&ts_epoch:gt=1", headers={"Host": "test"}
    )
    ...
    request = make_mocked_request("GET", "/?_tags:any=a,2", headers={"Host": "test"})
    ...
    request = make_mocked_request("GET", "/?_tags:all=a,2", headers={"Host": "test"})
    ...
    request = make_mocked_request("GET", "/?_tags:likeany=a", headers={"Host": "test"})
    ...
    request = make_mocked_request(
        "GET", "/?_tags:likeall=b,3", headers={"Host": "test"}
    )
    ...
    request = make_mocked_request(
        "GET", "/?nonexistent:eq=b,3", headers={"Host": "test"}
    )
    ...
    request = make_mocked_request("GET", "/?ts_epoch:gt=*#*#", headers={"Host": "test"})
    ...
    request = make_mocked_request("GET", "/", headers={"Host": "test"})
