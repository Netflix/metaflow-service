...
async def get_run_dag_data(
    db: AsyncPostgresDB, flow_name: str, run_number: str
) -> DBResponse:
    """
    ...
    """
    db_response = await db.artifact_table_postgres.get_run_graph_info_artifact(
        flow_name, run_number
    )
    if not db_response.response_code == 200:
        # Try to look for codepackage if graph artifact is missing
        db_response = await db.metadata_table_postgres.get_run_codepackage_metadata(
            flow_name, run_number
        )
