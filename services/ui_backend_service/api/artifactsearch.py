...
class ArtifactSearchApi(object):
    ...
    async def get_run_tasks(self, request):
        ...
        meta_artifacts = await self.get_run_artifacts(flow_name, run_number, artifact_name)
    ...
    async def get_run_artifacts(self, flow_name, run_number, artifact_name):
    ...
    for artifact in artifacts:
        ...
        if loc in artifact_match_dict:
            ...
            if match_data['matches'] or not match_data['included']:
                results.append({**result_format(artifact),
