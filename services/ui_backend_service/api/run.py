...
class RunApi(object):
    def __init__(self, app, db, cache=None):
        self.db = db
        app.router.add_route("GET", "/runs", self.get_all_runs)
        app.router.add_route("GET", "/flows/{flow_id}/runs", self.get_flow_runs)
        app.router.add_route("GET", "/flows/{flow_id}/runs/{run_number}", self.get_run)
        app.router.add_route(
            "GET",
            "/flows/{flow_id}/runs/{run_number}/parameters",
    ...
    async def get_all_runs(self, request):
        ...
        if has_tag_filter:
            ...
            allowed_unoptimized_order = [
                o for o in allowed_group if o not in allowed_optimized_order
            ]

            _, _, _, optimized_order, _, _ = pagination_query(
                request, allowed_order=allowed_optimized_order
            )
            _, _, _, unoptimized_order, _, _ = pagination_query(
                request, allowed_order=allowed_unoptimized_order
            )
            ...
            if optimized_order and not unoptimized_order:
                overwrite_select_from = (
                    "(SELECT * FROM {table_name} {order_by}) AS {table_name}".format(
    ...
    async def get_flow_runs(self, request):
        ...
        return await find_records(
            ...
            allowed_order=self._async_table.keys
            + ["user", "run", "finished_at", "duration", "status"],
            allowed_group=self._async_table.keys + ["user"],
            allowed_filters=self._async_table.keys
            + ["user", "run", "finished_at", "duration", "status"],
            enable_joins=True,
    ...
    async def get_run_parameters(self, request):
        ...
        if postprocess_error:
            raise GetParametersFailed(
                ...
                postprocess_error["id"],
                postprocess_error["traceback"],
            )
        else:
