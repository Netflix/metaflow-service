...
for service in ["metadata_service", "migration_service", "ui_backend_service"]:
    requirements += open_and_read_if_exists(
        "services/{}/requirements.txt".format(service)
    ).splitlines()

requirements_tests = open_and_read_if_exists("requirements.dev.txt").splitlines()

long_description = open_and_read_if_exists("README.md")
...
setup(
   ...
   """,
    ...
    extras_require={"test": requirements + requirements_tests},
    classifiers=[
